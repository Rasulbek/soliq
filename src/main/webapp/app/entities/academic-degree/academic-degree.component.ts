...
export class AcademicDegreeComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.academicDegreeService
      ...
      .subscribe(
        (res: HttpResponse<IAcademicDegree[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  ngOnInit(): void {
    ...
    this.registerChangeInAcademicDegrees();
  }
  ...
  trackId(index: number, item: IAcademicDegree): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInAcademicDegrees(): void {
    this.eventSubscriber = this.eventManager.subscribe('academicDegreeListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAcademicDegree[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/academic-degree'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
